...
func (f *FakeHeatMeter) Write(p []byte) (int, error) {
	...
	case application.CommandGetSerialNo:
		f.response = []byte{0x40, 0x3f, 0x02, 0x01, 0x23, 0x45, 0x67, 0xe9, 0x56, 0x0d}
		return len(p), nil
	case application.CommandGetRegister:
		f.response = []byte{0x40, 0x3f, 0x10, 0x00, 0x1b, 0x7f, 0x16, 0x04, 0x11, 0x01, 0x2a, 0xf0, 0x24, 0x63, 0x03, 0x0d}
		return len(p), nil
	default:
